...
export async function generateMetadata({ params }: { params: Promise<{ lang: Locale }> }): Promise<Metadata> {
    ...
    const title = dictionary.meta.businessPackageTitle || `${(dictionary.services.items as Record<string, any>)['business-package']?.title} | LegisPro`;
    const description = dictionary.meta.businessPackageDescription || (dictionary.services.items as Record<string, any>)['business-package']?.description;
    const localizedPath = getLocalizedPath('/podnikatelsky-balik', lang);
    const url = `${BASE_URL}${localizedPath}`;

    const alternateLanguages: Record<string, string> = {};
    for (const locale of i18n.locales) {
        alternateLanguages[locale] = `${BASE_URL}${getLocalizedPath('/podnikatelsky-balik', locale)}`;
    }
